...
| What the agent catches | ~16% of the model's **wrongful declines** (good borrowers it would have rejected) vs. only ~2% of its **bad approvals** (defaulters it let through) |
...
| Does grounding-checking help? | The verifier cuts the disagree rate ~12%→7% (statistically significant) and downgrades ~38% of otherwise-confident, conflicting stances as unsupported — but it catches *fewer* of the model's real errors in absolute terms, so the tradeoff is unresolved |
...
- **The agent's real value is rescuing wrongful declines, not catching missed defaulters.** Of the tabular model's own errors, disagreement-routing catches 16.41% [15.46%, 17.39%] of the loans it wrongly declined vs. 1.92% [1.31%, 2.80%] of the defaulters it wrongly approved — the note is much better at pushing back on over-caution than at raising a flag the numbers missed. *(These figures were previously reported as 24.05% / 5.90%, measured before the verifier node shipped in Build 9 and never updated; the verifier shrinks the review queue and so catches fewer errors in absolute terms. See DECISIONS.md Build 13.)*
...
- **Fair** (approval-rate shifts of −0.9pp to −1.3pp across demographic-proxy groups, no group exceeds the 5pp flag) **and calibrated** (auto-decided subset Brier 0.1174 vs. 0.1213 for the full population — the automated cases are slightly better-calibrated, not worse). *(Shifts were previously −2.6pp to −3.8pp and Brier 0.1157, both pre-verifier; the smaller review queue moves fewer decisions, so the shifts shrink — see DECISIONS.md Build 13.)*
- **Within budget.** Review rate lands at 6.86% of all loans against a ~15% assumed budget (previously reported as 11.57%, a pre-verifier measurement — see DECISIONS.md Build 13).
...
| Verifier | A second, independent LLM pass that checks the word-reader's own quoted evidence and cited policy actually support its stance — mechanical check first (free): each quote must appear in the note, ignoring only surrounding quote characters and letter case, and each cited policy id must be one actually retrieved, matched exactly; LLM check second; safe-direction only (downgrades to neutral, never escalates). |
...
- **A production bug in the verifier itself, found by an experiment and fixed.** The verifier's free mechanical layer tested quoted evidence with an exact substring match, so a model returning an otherwise-perfect quote wrapped in its own quote characters — or with one letter re-cased — had its stance marked unsupported and downgraded to neutral. Punishing quoting style, not over-claiming. The correct check already existed in the repo, written for the eval harness's grounding report, which means production and evaluation had disagreed about the same span since the verifier shipped. A re-scoring experiment (`experiments/reasoning_stack_spanfix.py`) isolated it on 2,000 cached stances: correcting only that one test moved two experimental prompts' unsupported rates 41%→19% and 42%→20%, CI-separated. **In production the same fix moved almost nothing** — ~15 stances in 2,500, no metric CI-separated — because the shipped prompt rarely quote-wraps, and the big swings belonged to prompts that never shipped. Both facts are worth stating: the bug was real, and its shipped impact was small. A regression test now pins the boundary, asserting a high-word-overlap paraphrase still fails.
